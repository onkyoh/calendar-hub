...
const ContactsBar = (props) => {
  ...
  const {contactId, handleContactId, addContact} = useAddContact(props.contacts)
  const {deletingId, toggleDelete} = useIsDeleting()
  const {show, toggleShow} = useShow()

  const userContactInfo = {
    usersName: props.displayName,
    usersCalendar: props.calendar2023,
    ...
    <aside className={show.bar ? 'slide-up' : ''}>
      <section id='profile'>
        <div>
          ...
          <button onClick={() => toggleShow('logout')}><LeaveIcon/></button>
        </div>
        <div>
          <p onClick={() => {navigator.clipboard.writeText(props.userId)}}>id:{props.userId}<CopyIcon/></p>
        </div>
        <div>
          <input type="text" value={contactId} onChange={(e) => handleContactId(e)}/>
          <button onClick={() => addContact(userContactInfo, contactId)}><AddIcon/></button>
        </div>
      </section>
      ...
      {show.logout &&
        <div>
            ...
            <button onClick={() => logout(props.handleCurrentUser)}>logout</button>
      ...
      {deletingId && 
        <div className='modals'>
          ...
          <p>are you sure you would like to delete this contact? once you do, you will not be able to undo this action.</p>
          <button onClick={() => deleteContact(props.userId, deletingId, toggleDelete)}>confirm</button>
      ...
      <section id='contacts'>
        <div id='requests'>
          <p onClick={() => toggleShow('requests')}>{show.requests ? '-' : '+'} requests</p>
          {show.requests &&
            <ul>
        ...
        <div id='added'>
          <p onClick={() => toggleShow('added')}>{show.added ? '-' : '+'} contacts </p>
          {show.added && 
            <ul>
        ...
        <div id='toggle'>
          <button onClick={() => toggleShow('bar')}>{show.bar ? <DownIcon/> : <UpIcon/> }</button>
        </div>
      </section>
    </aside>
  )
